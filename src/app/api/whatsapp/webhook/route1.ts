...
export async function POST(req: NextRequest) {
  ...
  if (state === "ASK_NAME") {
    ...
    // continua para categorias (com fallback blindado)
    try {
      return await sendCategoryMenu({ ...nextCtx, offset: 0 }, 0, `Perfeito, ${proposed} 😊`);
    } catch (e) {
      console.error("ASK_NAME -> sendCategoryMenu failed", e);
      await replyAndLog(`Perfeito, ${proposed} 😊\n\nAgora me diz: qual serviço você quer? (responde com o nome)`, { step: "ask_name_fallback_service" });
      await setSession("ASK_SERVICE_TEXT", { ...nextCtx });
      return NextResponse.json({ ok: true });
    }
    } catch (e) {
      console.error("ASK_NAME handler failed", e);
      await replyAndLog("Desculpa — tive um erro aqui 🙏\nQual é o teu nome? (ex: João)", { step: "ask_name_exception" });
      return NextResponse.json({ ok: true });
    }
  }

  // fallback simples: serviço por texto (quando categorias falham)
  if (state === "ASK_SERVICE_TEXT") {
    const s = String(message.text?.body ?? "").trim();
    if (!s) {
      await replyAndLog("Qual serviço você quer? (ex: Corte, Barba)", { step: "ask_service_text_retry" });
      return NextResponse.json({ ok: true });
    }
    // tenta achar serviço pelo nome
    ...
    const list = (services ?? []) as any[];
    if (!list.length) {
      await replyAndLog("Não encontrei esse serviço 😕\nEscreve de novo o nome do serviço.", { step: "ask_service_text_not_found" });
      return NextResponse.json({ ok: true });
    }

    const pick = list[0];
    const nextCtx = { ...(ctx?.next_ctx as any), service_id: pick.id, service_name: pick.name, duration_minutes: Number(pick.duration_minutes ?? 30), price_cents: Number(pick.price_cents ?? 0), currency: String(pick.currency ?? "EUR") };
    await setSession("ASK_DAY", nextCtx);
    await replyAndLog(`Ok ✅ *${pick.name}*\nQual dia você prefere? (ex: hoje / amanhã / 15/02)`, { step: "ask_service_text_ok" });
    return NextResponse.json({ ok: true });
  }
...
const nextCtx = {
  ...
  // PROMO: customer replied to a promotion
  if (state === "PROMO_OFFER") {
    if (text === "NAO" || text === "NÃO") {
      await clearSession();
      await replyAndLog("Sem problema 😊 Quando quiseres marcar, escreve: *QUERO MARCAR*.", { step: "promo_decline" });
      return NextResponse.json({ ok: true });
    }

    if (text !== "SIM") {
      await replyAndLog("Para aproveitar a promoção, responde *SIM* (ou *NÃO*).", { step: "promo_retry" });
      return NextResponse.json({ ok: true });
    }

    const promoIso = String(ctx?.promo?.isoDate ?? "");
    if (!promoIso) {
      await clearSession();
    ...
    const header = "Perfeito ✅\nA promoção é para *" + formatDatePt(promoIso) + "*.\nAgora escolhe a categoria:";
    const promoCtx = { promo: { isoDate: promoIso, locked: true } };
    // Start normal flow but keep promo date locked
    await setSession("ASK_CATEGORY", { mode: "NEW", offset: 0, ...promoCtx });
    await replyAndLog(header, { step: "promo_accept", promoIso });
    return await sendCategoryMenu({ mode: "NEW", offset: 0, ...promoCtx }, 0, header);
  }

if (state === "ASK_DAY") {
    // Promo: day is fixed by the promotion
    if (ctx?.promo?.locked && ctx?.promo?.isoDate) {
      return await processDaySelection(String(ctx.promo.isoDate));
    }
